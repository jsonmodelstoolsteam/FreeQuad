...
import application.stages.components.*;
...
public class StageChangeModel implements InitComponentV2 {
    ...
    public boolean onShow(Helper helper, Object... params) {
        ...
        if (catalog == null) {
            File direct = helper.initDirect(new File(System.getProperty("user.home")), "Выберите папку с файлами");
    ...
    public void reloadSession(Helper helper, boolean hide) {
        if (!hide) {
            File direct = helper.initDirect(catalog, "Выберите папку с файлами");
